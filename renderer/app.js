...
async function fetchPreview() {
    ...
    try {
        ...
        showDownloadStatus(
            "No se pudo obtener info: " + (err.message || err),
            "error",
        );
    } finally {
...
async function startDownload() {
    ...
    if (!currentPreview)
        return showDownloadStatus("Primero haz vista previa.", "error");
...
function renderFiles() {
    ...
    filtered.forEach((file) => {
        ...
        icon.innerHTML =
            '<circle cx="12" cy="12" r="9" stroke="currentColor" stroke-width="1.5"/><circle cx="12" cy="12" r="3" fill="currentColor"/><path d="M12 2v4M12 18v4M2 12h4M18 12h4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/>';
...
function toggleSelectAll() {
    ...
    document.getElementById("selectAllBtn").textContent = allSelected
        ? "Deseleccionar todo"
        : "Seleccionar todo";
    updateMoveBtn();
}
...
async function moveSelected() {
    ...
    try {
        ...
        if (failed.length === 0) {
            showMoveStatus(
                `✓ ${results.length} archivo${results.length !== 1 ? "s" : ""} copiado${results.length !== 1 ? "s" : ""} correctamente.`,
                "success",
            );
        } else {
            showMoveStatus(
...
async function deleteSelected() {
    ...
    if (
        !confirm(
            `¿Eliminar ${n} archivo${n !== 1 ? "s" : ""}? Esta acción no se puede deshacer.`,
        )
    )
        return;
    ...
    try {
        ...
        if (failed.length === 0) {
            showMoveStatus(
                `✓ ${results.length} archivo${results.length !== 1 ? "s" : ""} eliminado${results.length !== 1 ? "s" : ""}.`,
                "success",
            );
        } else {
            showMoveStatus(
                `${results.length - failed.length} eliminado(s). ${failed.length} error(es).`,
                "error",
            );
        }
...
document
    .getElementById("searchHistoryInput")
    .addEventListener("input", () => renderHistory(allHistory));
